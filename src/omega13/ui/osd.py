...
class OSDManager:
        ...
        if not self._started:
            ...
            try:
                ...
                self._subprocess = subprocess.Popen(
                    [sys.executable, "-m", "omega13.osd_process"],
                    env=env
                )
            except Exception as e:
                logger.error(f"Failed to start OSD subprocess: {e}")
    ...
    def stop_subprocess(self):
        ...
        if self._subprocess:
            ...
            try:
                ...
                logger.warning("OSD subprocess did not terminate, killing it.")
                self._subprocess.kill()
            self._subprocess = None

    async def _get_dbus_proxy(self):
    ...
    async def _stream_waveform(self):
        proxy = await self._get_dbus_proxy()
        if not proxy:
            return
            
        while self._started:
            try:
                if self.audio_engine and hasattr(self.audio_engine, 'signal_detector'):
                    import numpy as np
    ...
    def update(self, text: str, state_type: str = "normal", timeout_ms: int = 0):
        ...
        # Waveform streaming in GNOME
        if is_gnome and self._loop:
            if state_type in ("recording", "processing"):
                if not self._waveform_task or self._waveform_task.done():
                    self._waveform_task = self._loop.create_task(self._stream_waveform())
            elif self._waveform_task:
                self._waveform_task.cancel()
                self._waveform_task = None
                
        # Fallback to notifications if no subprocess and (no GNOME or forced)
